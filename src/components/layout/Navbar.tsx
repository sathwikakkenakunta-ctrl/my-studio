  ...
  return <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-7 sm:pt-4">
    <nav className={`glossy-header mx-auto flex min-h-16 max-w-[1380px] items-center justify-between rounded-full px-4 sm:px-5 ${scrolled ? 'is-scrolled' : ''}`} aria-label="Primary navigation">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button to="/contact" variant="primary" showIcon={false} className="hidden min-h-11 whitespace-nowrap px-5 text-xs md:inline-flex">Let's talk</Button>
    ...
    <AnimatePresence>{open && <motion.div id="mobile-menu" initial={reduceMotion ? false : {opacity:0,y:-12,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={reduceMotion ? undefined : {opacity:0,y:-12,scale:.98}} className="mx-auto mt-2 max-h-[calc(100dvh-6rem)] max-w-[1380px] overflow-y-auto rounded-3xl border border-[var(--glass-border)] bg-[var(--surface)] p-5 shadow-2xl md:hidden">{nav.map(([label,path]) => <NavLink key={path} to={path} onClick={() => setOpen(false)} className={({isActive}) => `focus-ring block rounded-lg border-b border-[var(--border)] px-2 py-4 font-display text-2xl ${isActive ? 'text-violet' : ''}`}>{label}</NavLink>)}<span onClick={() => setOpen(false)}><Button to="/contact" variant="primary" className="mt-5 w-full">Let's work together</Button></span></motion.div>}</AnimatePresence>
  </header>;
}
