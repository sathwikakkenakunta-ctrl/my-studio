...
export function Layout() {
  ...
  return <><a href="#main" className="focus-ring fixed left-3 top-3 z-[70] -translate-y-20 rounded-full bg-cyan px-4 py-2 font-bold text-ink transition focus:translate-y-0">Skip to content</a><div className="noise"/><Navbar/><main id="main"><motion.div key={pathname} initial={reduceMotion ? false : { opacity: .65, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : .32, ease: [.22,1,.36,1] }}><Outlet/></motion.div></main><Footer/></>;
}
